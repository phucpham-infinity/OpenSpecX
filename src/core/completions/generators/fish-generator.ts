...
export class FishGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      topLevelLines.push(
        `complete -c openspecx -n '__fish_openspec_no_subcommand' -a '${cmd.name}' -d '${this.escapeDescription(cmd.description)}'`
      );
    }
...
${commandCompletions}`;
  ...
  private generateCommandCompletions(cmd: CommandDefinition): string[] {
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        lines.push(
          `complete -c openspecx -n '__fish_openspec_using_subcommand ${cmd.name}; and not __fish_openspec_using_subcommand ${subcmd.name}' -a '${subcmd.name}' -d '${this.escapeDescription(subcmd.description)}'`
        );
      }
      lines.push('');
  ...
  private generateFlagCompletion(flag: FlagDefinition, condition: string): string[] {
    ...
    if (flag.takesValue && flag.values) {
      ...
      for (const value of flag.values) {
        if (shortFlag) {
          lines.push(
            `complete -c openspecx -n '${condition}' -s ${flag.short} -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        } else {
          lines.push(
            `complete -c openspecx -n '${condition}' -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        }
      }
      ...
      if (shortFlag) {
        lines.push(
            `complete -c openspecx -n '${condition}' -s ${flag.short} -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
            `complete -c openspecx -n '${condition}' -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      }
    } else {
      // Boolean flag
      if (shortFlag) {
        lines.push(
            `complete -c openspecx -n '${condition}' -s ${flag.short} -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
            `complete -c openspecx -n '${condition}' -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      }
    }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, condition: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`complete -c openspecx -n '${condition}' -a '(__fish_openspec_changes)' -f`);
        break;
      case 'spec-id':
        lines.push(`complete -c openspecx -n '${condition}' -a '(__fish_openspec_specs)' -f`);
        break;
      case 'change-or-spec-id':
        lines.push(`complete -c openspecx -n '${condition}' -a '(__fish_openspec_items)' -f`);
        break;
      case 'shell':
        lines.push(`complete -c openspecx -n '${condition}' -a 'zsh bash fish powershell' -f`);
        break;
      case 'path':

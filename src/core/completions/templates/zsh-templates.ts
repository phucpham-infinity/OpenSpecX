...
# Use openspecx __complete to get available changes
_openspec_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(openspecx __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use openspecx __complete to get available specs
_openspec_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(openspecx __complete specs 2>/dev/null)
